...
On first session start, you are prompted to install the two grammars
needed for markdown rendering.  A second prompt offers ~20 additional
grammars for syntax highlighting inside code blocks (Python,
JavaScript, Rust, etc.).  Both prompts appear at most once — if you
decline, the choice is remembered.
